...
class ExportFormatsListAPI(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        return Project.objects.filter(organization__in=self.request.user.active_organizations.values_list('id', flat=True))
...
class ProjectExportFilesAuthCheck(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        generics.get_object_or_404(Project.objects.filter(organization__in=self.request.user.active_organizations.values_list('id', flat=True)), pk=pk)

...
class TaskListAPI(DMTaskListAPI):
    ...
    def filter_queryset(self, queryset):
        ...
        return queryset.filter(project__organization__in=self.request.user.active_organizations.values_list('id', flat=True))
...
class TaskAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        lean_task = generics.get_object_or_404(
            Task.objects.filter(project__organization__in=self.request.user.active_organizations.values_list('id', flat=True)).select_related('project'),
            pk=task_id,
        )
...
class PredictionAPI(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Prediction.objects.filter(project__organization__in=self.request.user.active_organizations.values_list('id', flat=True))
